-- COMPLETE COMBAT SYSTEM SETUP SCRIPT - FIXED VERSION
...
local function setupCombatSystem()
    ...
    -- Clean up any existing combat system
    if serverScriptService:FindFirstChild("Combat") then serverScriptService.Combat:Destroy() end
    if serverScriptService:FindFirstChild("Handlers") then serverScriptService.Handlers:Destroy() end
    if serverScriptService:FindFirstChild("Validation") then serverScriptService.Validation:Destroy() end
    if replicatedStorage:FindFirstChild("RemoteEvents") then replicatedStorage.RemoteEvents:Destroy() end
    if replicatedStorage:FindFirstChild("CombatData") then replicatedStorage.CombatData:Destroy() end
    if replicatedStorage:FindFirstChild("Modules") then replicatedStorage.Modules:Destroy() end
    ...
    -- Clean up existing client scripts
    if starterPlayerScripts:FindFirstChild("Combat") then starterPlayerScripts.Combat:Destroy() end
    if starterPlayerScripts:FindFirstChild("UI") then starterPlayerScripts.UI:Destroy() end
    if starterPlayerScripts:FindFirstChild("Effects") then starterPlayerScripts.Effects:Destroy() end
    ...
    if starterGui:FindFirstChild("CombatUI") then starterGui.CombatUI:Destroy() end
    ...
    -- CombatConstants.lua (Create this first as it's required by other modules)
...
function DamageHandler:ProcessAttack(attacker, attackType, comboCount, playerCombatStates)
    ...
    -- Simple overlap check instead of Region3
    local hitPlayers = {}
        ...
        if player ~= attacker and player.Character then
            ...
            if targetRootPart then
                ...
                if distance <= attackData.range then
                    ...
                    if dotProduct > 0.5 then -- Target is in front
                        local targetHandler = playerCombatStates[player]
                        ...
                        if targetHandler then
                            ...
                            damageDealtRemote:FireClient(player, {
                                attacker = attacker,
                                damage = result.damage,
                                blocked = result.blocked,
                                dodged = result.dodged,
...
return DamageHandler]]
    ...
    -- CombatHandler.lua (Main server script)
...
local function onCombatAction(player, action, data)
    ...
    elseif action == "attack" then
        ...
        if success then
            ...
            DamageHandler:ProcessAttack(player, data.type, handler.state.comboCount, playerCombatStates)
        end
    end
end
...
_G.PlayerCombatStates = playerCombatStates -- Make accessible for debugging

return CombatHandler]]
    combatHandlerScript.Parent = combatFolder
    
    -- CombatInput.lua (Client input handler)
...
local function updateUIState(state)
    ...
    if combatUI then
        local stateIndicator = combatUI:FindFirstChild("StateIndicator")
        if stateIndicator then
            local stateText = stateIndicator:FindFirstChild("StateText")
            if stateText then
                if state == "combat" then
                    stateText.Text = "COMBAT"
                elseif state == "blocking" then
                    stateText.Text = "BLOCKING"
...
local function endBlock()
    ...
    updateUIState(combatState.inStance and "combat" or "idle")
end
...
local function handleRunning()
    ...
    if timeSinceLastPress <= CombatData.DOUBLE_TAP_WINDOW then
        if not combatState.isRunning and combatState.inStance then
...
return CombatEffects]]
    ...
    print("F - Block/Parry")
    print("Q - Dodge")
    print("M1 - Basic attack")
    print("R - Critical attack") 
    print("Double-tap W - Run (while in combat stance)")
end
